...
module BPanelHoles() {
    // CylinderHole(1,
    //               LeftEdgeOfBoardWRTBPanel + 1.4,
    //               TopOfBoardWRTPanel + 1,
    //               0 + PartMargin );
    SquareHole(1,
               Width / 2 - 7.5,
               TopOfBoardWRTPanel,
        ...
        translate([PCBLength / 2, PCBWidth/2, PCBThick]) {
            color("Olive") {
                linear_extrude(height=FontThick) {
                    text("ESP-WROOM-32", size=4, font="Arial black", halign="center", valign="center");
                }
            }
        }
    }
...
/*  Panel module
    ...
    Takes no arguments but uses the global parameters.
*/
module Panel() {
